package com.nipuream.library.utils;
...
public class ParseUtil {
    ...
    public  static byte[] parseIntToHexInt(int value,int length){
        ...
        if(value > 15){
            ...
            result = ParseUtil.str2Bcd(hexStr,length);
        } else {
            byte[] tmp = ParseUtil.int2BytesH(value,length);
    ...
    public static String asciiToStr_l(byte[] bytes){
        try {
            int length = 0;
            for (int i = 0; i < bytes.length; ++i) {
                if (bytes[i] == 0) {
            ...
            return new String(bytes, 0, length, "GB2312");
        } catch (Exception e) {
            return "";
        }
